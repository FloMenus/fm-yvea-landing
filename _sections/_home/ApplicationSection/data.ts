export const applicationData = [
  {
    text: [
      {
        title: "Vérification illimitée des produits soumis à contrôle",
        description:
          "Trouvez le code douanier et évaluez la conformité en un clin d'oeil, 24h/24 & 7j/7 !",
      },
      {
        title: "Estimation instantanée du coût du certificat",
        description:
          "Éliminez les surprises en simulant le coût dès la négociation avec vos clients",
      },
    ],
    image: "/fm-yvea-landing/medias/application_illustrations/certification_cost.png",
  },
  {
    text: [
      {
        title: "Suivi transparent de vos dossiers",
        description:
          "Centralisez et suivez l'historique de vos demandes en un seul endroit intuitif",
      },
      {
        title: "Réutilisation intelligente des informations et documents",
        description:
          "Gagnez du temps en retrouvant et en réutilisant facilement vos données précédentes",
      },
    ],
    image: "/fm-yvea-landing/medias/application_illustrations/history.png",
  },
  {
    text: [
      {
        title: "Indexation rapide des documents requis par destination",
        description:
          "Simplifiez vos formalités en accédant directement aux documents nécessaires",
      },
    ],
    image: "/fm-yvea-landing/medias/application_illustrations/documents.png",
  },
];

// Timer for the application section, in ms (1000ms = 1s)
export const applicationTimer = 7000;
